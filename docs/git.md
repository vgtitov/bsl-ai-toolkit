...
### Гейт метаданных 1С (репозитории с исходниками EDT)

Вместе с хуками ставится `check_metadata_regression.py` — он кладётся рядом с ними, поэтому работает
в любом репозитории с исходниками EDT **без настройки в самом репозитории**. Перед push он сравнивает
`.mdo` и `.rights` с базовой веткой (`origin/develop`, иначе `origin/main`) и блокирует отправку, если
из метаданных или прав что-то пропало: объект, реквизит, форма, право — или у права тихо изменилось
значение.

Зачем: IDE держит модель конфигурации в памяти и при сохранении переписывает файл целиком. Если модель
старее ветки, на диск уходит устаревшее содержимое — для git это обычная правка, а не конфликт, так что
слияние проходит зелёным и потерю замечают недели спустя. `git log -- <файл>` её тоже не показывает.
...
```bash
git config hooks.metadataGate false                                  # выключить в этом репозитории
git config --add hooks.metadataGateForbiddenPath "src/old-layout/"   # каталог, который не должен вернуться
git config --global hooks.metadataGatePython "/c/Python313/python.exe"  # если IDE запускает хуки с урезанным PATH
```

Гейт сравнивает с **локальным** состоянием базовой ветки, поэтому перед push имеет смысл `git fetch`:
устаревший снимок даёт ложное «чисто». Если в самом репозитории уже лежит
`scripts/check_metadata_regression.py`, глобальный хук в него не лезет — работает версия репозитория.
