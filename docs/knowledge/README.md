...
## Домен: структурные правки метаданных и СКД — `core/skills/1c-metadata/`
Программное редактирование метаданных, форм, макетов и схем компоновки данных. Что покрыто и где раскрыто:
- **Поля СКД в группировках** — `core/skills/1c-metadata/references/skd-fields-in-groupings.md`: правило
  допустимости поля в выбранных полях группировки (поле группировки | реквизит поля группировки | ресурс),
  механизм полей-реквизитов (`dataPath` через точку), параметр `РасположениеРеквизитов`, почему ресурс в
  структуре «Таблица» уходит в ячейки пересечения, матрица выбора приёма, офлайн-проверки схемы.
- **Объект расширения в типовых механизмах БСП** —
  `core/skills/1c-metadata/references/bsp-extension-attachable-objects.md`: почему
  `СведенияОВнешнейОбработке()` не работает для объекта расширения, подсистема `ПодключаемыеОтчетыИОбработки`,
  контракт `ПриОпределенииНастроек`, рецепты печатной формы с макетом Word и вывода отчёта в панель «Отчеты».

## Что это за знание и как его помечать

Это **собственная база знаний**, собранная и переработанная из разных источников. Всё в
`references/*.md` изложено **своими словами**: факты, команды, параметры, пороги, чек-листы и методики
переработаны оригинально, без дословного копирования.

**Атрибуция не пишется.** Ни названий, ни авторов, ни категорий источников — откуда знание пришло, не
является свойством знания и читателю ничего не даёт.

Помечается другое — **степень достоверности**, и только там, где это меняет решение читателя:

| Пометка | Когда ставить | Что означает |
|---|---|---|
| *(без пометки)* | обычный случай | устоявшаяся практика |
| «по официальной документации» | поведение платформы/библиотеки, зафиксированное вендором | можно опираться как на контракт |
| «проверено экспериментом» | вывод получен прогоном на реальной системе | воспроизводимо; на своей версии стоит перепроверить |
| «эмпирически, на одной системе» | наблюдение без обобщения | может не повториться в другой среде |
| `[проверить]` | предположение | **нельзя** использовать как основание |

Общее правило всех ролей: **источник истины — реальный инструмент и измерение**, а не память модели.
Сначала снять данные — потом вывод.

**Данных компаний, клиентов, внутренних хостов и имён баз в репозитории нет и быть не должно** —
репозиторий публичный. Настройки конкретных организаций живут в приватных репозиториях настроек.
