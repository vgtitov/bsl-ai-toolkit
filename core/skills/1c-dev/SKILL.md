...
description: >
  ...
  базы (посчитать записи, прочитать регистр, подключиться и проверить через OData) или доказать, что разработка
  реально работает (смоук/тест) — тоже сюда: см. `docs/data-access-architecture.md` и `docs/testing-ladder.md`
  ПЕРЕД тем, как изобретать доступ вручную (COM/скрипты). Железное правило: НЕ
