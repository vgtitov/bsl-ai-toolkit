...
⚠ Но развилка не по слову «данные», а по задаче. Нужно РАЗОВО прочитать значение или посчитать
(что лежит в константе-настройке, сколько записей, заполнен ли реквизит) — это **ступень 2**
выше, батч ENTERPRISE + раннер-EPF, и она же ЕДИНСТВЕННЫЙ канал, когда публикации у базы нет.
Ни публикации, ни отладочного расширения, ни прав администратора кластера (`rac`) для такого
чтения не нужно: строка соединения `Srvr=…;Ref=…` клиентская, ей хватает прав обычного
пользователя ИБ. В документ доступа к данным идти, когда нужен ПОВТОРЯЕМЫЙ или интерактивный
диалог, чтение под конкретным пользователем либо сам HTTP-контракт.
...
изобретать доступ вручную — вызывай `1c-tester`. Разовое чтение значения или счётчика он
закроет ступенью 2 (батч ENTERPRISE + раннер-EPF, без публикации и без отладочного
расширения), а повторяемый и интерактивный доступ — по `references/data-access-verification.md`
(автономный сервер, обязательные поправки на кодировку кириллицы, диагностика по коду
ответа); не копируй рецепт к себе, он будет расходиться версией.
