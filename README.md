...
После обновления toolkit повторно запускай тот же onboarding: он идемпотентно
обновляет сгенерированные конфиги, скиллы и принадлежащие toolkit git-hooks.
Если менялись MCP, skills или provider-specific settings, после onboarding
перезапусти используемый агент. Канон доставки и проверки — `docs/AI_UPDATE.md`.
...
- [docs/RELEASING.md](docs/RELEASING.md) — как выпускать релизы (версии, сборка расширения, решение по Git LFS).
